...
func (g *PostgreSQLGarbageCollector) Clean(postgresql *infrav1beta1.PostgreSQL) error {
	rootPassword, err := g.cw.GetRootPassword(postgresql.Status.DatabaseStatus.RootSecretLookup.Name, postgresql.Status.DatabaseStatus.RootSecretLookup.Namespace,
		postgresql.Status.DatabaseStatus.RootSecretLookup.Field)
	if err != nil {
		// no point in proceeding. In future could also try with Spec credential
		return err
	}
	postgreSQLServer, err := g.r.ServerCache.Get(postgresql.Status.DatabaseStatus.Host, postgresql.Status.DatabaseStatus.RootUsername, rootPassword,
		postgresql.Status.DatabaseStatus.RootAuthenticationDatabase)
	if err != nil {
		// no point in proceeding. In future could also try with Spec credential
		return err
	}
	...
	errToReturn = g.HandleHostOrDatabaseChange(postgreSQLServer, postgresql)
...
func (g *PostgreSQLGarbageCollector) HandleHostOrDatabaseChange(postgreSQLServer *postgresqlAPI.PostgreSQLServer, postgresql *infrav1beta1.PostgreSQL) error {
	var errToReturn error
	if (postgresql.Status.DatabaseStatus.Host != "" && postgresql.Spec.HostName != postgresql.Status.DatabaseStatus.Host) ||
		...
		postgresql.Status.CredentialsStatus.ForEach(func(status *infrav1beta1.CredentialStatus) {
			err := postgreSQLServer.DropUser(postgresql.Status.DatabaseStatus.Name, status.Username)
			if err != nil {
				errToReturn = err
			} else {
...
func (g *PostgreSQLGarbageCollector) HandleUnneededCredentials(postgresql *infrav1beta1.PostgreSQL, postgreSQLServer *postgresqlAPI.PostgreSQLServer) error {
	...
	postgresql.RemoveUnneededCredentialsStatus().ForEach(func(status *infrav1beta1.CredentialStatus) {
		errToReturn = postgreSQLServer.DropUser(postgresql.Status.DatabaseStatus.Name, status.Username)
	})
	return errToReturn
}
