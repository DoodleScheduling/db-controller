...
var _ = Describe("MongoDBUserReconciler", func() {
	...
	Context("MongoDBUser", func() {
		...
		Describe("User created", func() {
			var (
				db         *infrav1beta1.MongoDBDatabase
				user       *infrav1beta1.MongoDBUser
				keyUser    types.NamespacedName
			...
			It("Changes password for existing user", func() {
				password := randStringRunes(5)
				userSecret.Data = map[string][]byte{
					"username": []byte(user.ObjectMeta.Name),
					"password": []byte(password),
				}
				Expect(k8sClient.Update(context.Background(), userSecret)).Should(Succeed())
				By("Expecting user ready")
				got := &infrav1beta1.MongoDBUser{}
				Eventually(func() bool {
					_ = k8sClient.Get(context.Background(), keyUser, got)
				...
				Eventually(func() error {
					...
					//_, err = client.Database(db.ObjectMeta.Name).Collection(randStringRunes(5)).InsertOne(ctx, bson.D{})
					//Expect(err).NotTo(HaveOccurred(), "failed insert")
